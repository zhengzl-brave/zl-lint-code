 ...
 * @param cwd
 * @param pkg
 * @returns zl-eslint-config/index
 * @returns zl-eslint-config/react
 * @returns zl-eslint-config/typescript/index
 * @returns zl-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'zl-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}

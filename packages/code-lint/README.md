...
| 文档规范                                                          | [markdownlint](https://github.com/DavidAnson/markdownlint) | [zl-markdownlint-config](https://www.npmjs.com/package/zl-markdownlint-config) |

可以看到这些 `Linter` 和规则包众多且零散，全部安装它们会给项目增加十几个依赖，接入和升级成本都比较高。

`zl-code-lint` 收敛屏蔽了这些依赖和配置细节，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡口，降低项目接入规范的成本。
...
```bash
npm install zl-code-lint -g
```

安装完成后，可执行 `zl-code-lint -h` 以验证安装成功。

### 使用

#### `zl-code-lint init`：一键接入

在项目根目录执行 `zl-code-lint init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。

具体会做以下事情：

- 安装各种依赖：包括 `Linter` 依赖，如 [ESLint](https://eslint.org/)、[stylelint](https://stylelint.io/)、[commitlint](https://commitlint.js.org/#/)、[markdownlint](https://github.com/DavidAnson/markdownlint) 等；配置依赖，如 [zl-eslint-config](https://www.npmjs.com/package/zl-eslint-config)、[zl-stylelint-config](https://www.npmjs.com/package/zl-stylelint-config)、[zl-commitlint-config](https://www.npmjs.com/package/zl-commitlint-config)、[zl-markdownlint-config](https://www.npmjs.com/package/zl-markdownlint-config) 等
- 写入各种配置文件，包括：
  - `.eslintrc.js`、`.eslintignore`：ESLint 配置（继承 `zl-eslint-config`）及黑名单文件
  - `.stylelintrc.js`、`.stylelintignore`：stylelint 配置（继承 `zl-stylelint-config`）及黑名单文件
  - `commitlint.config.js`：commitlint 配置（继承 `zl-commitlint-config`）
  ...
  - `zl-code-lint.config.js`zl-code-lint 包的一些配置，如启用的功能等
- 配置 git commit 卡口：使用 [husky](https://www.npmjs.com/package/husky) 设置代码提交卡口，在 git commit 时会运行 `zl-code-lint commit-file-scan` 和 `zl-code-lint commit-msg-scan` 分别对提交文件和提交信息进行规范检查。`zl-code-lint commit-file-scan` 默认仅对 error 问题卡口，如果你想对 warn 问题也卡口，可以增加 `--strict` 参数以开启严格模式

> 注 1：如果项目已经配置过 ESLint、stylelint 等 Linter，执行 `zl-code-lint init` 将会提示存在冲突的依赖和配置，并在得到确认后进行覆盖：
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `zl-code-lint init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件

#### `zl-code-lint scan`：一键扫描
...
> 注 1：事实上，你可以在任意目录执行 `zl-code-lint scan` `zl-code-lint` 会根据文件类型、JSON 等特征嗅探项目类型。但我们还是推荐在执行过 `zl-code-lint init` 的项目根目录执行 `zl-code-lint scan`，以得到最准确的扫描结果。
>
> 注 2: `zl-code-lint` 会根据项目内有无 eslint 和 stylelint 配置文件判断使用项目的配置文件还是 `zl-code-lint` 默认配置进行扫描。若使用项目的，在未安装依赖时会帮其安装（执行 npm i）。若使用项目配置扫描失败，则使用默认配置扫描

#### `zl-code-lint fix`：一键修复
...
#### `zl-code-lint commit-file-scan` 提交文件扫描
...
#### `zl-code-lint commit-msg-scan` 提交信息扫描
...
- zl-code-lint.init(config)：将项目一键接入规范，效果等同于 `zl-code-lint init`
...
`zl-code-lint` 基于一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
...
`zl-code-lint` 会读取执行目录下的 `zl-code-lint.config.js` 作为配置文件。`zl-code-lint init` 会在执行目录下新增如下的 `zl-code-lint.config.js` 文件：
...
如果你的 TS 项目 commit 卡口和 `zl-code-lint scan` 运行时间很长，可以通过如下在 `.eslintrc.js` 中增加以下配置提升性能：
